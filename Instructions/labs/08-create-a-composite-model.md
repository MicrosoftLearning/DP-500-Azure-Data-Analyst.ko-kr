...
10. **다른 이름으로 저장** 창에서 **D:\DP500\Allfiles\08\MySolution** 폴더로 이동합니다. 파일 이름은 **Sales Analysis - Create a composite model.pbix**입니다.
...
1. 측정값을 만들려면 오른쪽에 있는 **데이터** 창에서 **Targets** 테이블을 마우스 오른쪽 단추로 클릭한 다음, **새 측정값**을 선택합니다.
...
1. **데이터** 창의 **Targets** 테이블 내에서 두 개의 측정값이 있는지 확인합니다.
...
1. **데이터** 창의 **Target Amount** 필드를 **시각화** 창에 있는 **값** 웰 내의 **Sales Amount** 필드 바로 아래로 끌어옵니다.
