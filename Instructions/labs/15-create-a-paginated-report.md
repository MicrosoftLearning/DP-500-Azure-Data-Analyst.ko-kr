...
이 연습에서는 Power BI 보고서 작성기 열어 보고서를 만든 다음 저장합니다.
...
이 작업에서는 Power BI 보고서 작성기 열어 보고서를 만든 다음 저장합니다.
...
1. 보고서 디자이너에서 본문 영역과 보고서 바닥글 영역으로 구성된 기본 보고서 레이아웃을 볼 수 있습니다.
    ...
    *본문에는 보고서 제목에 대해 준비된 단일 텍스트 상자가 포함되며 보고서 바닥글에는 보고서 실행 시간을 설명하는 단일 텍스트 상자가 포함됩니다.*
...
8. 보고서 제목 텍스트 상자에 포커스를 설정하려면 먼저 텍스트 상자 외부 영역을 선택한 다음 텍스트 상자를 다시 선택합니다.
    ...
    *텍스트 상자의 테두리가 강조 표시되고 테두리에 핸들 크기 조정(작은 원)이 표시되면 텍스트 상자가 선택됩니다.*
...
10. 데이터 세트를 만들려면 **보고서 데이터** 창에서 **AdventureWorksDW2022** 데이터 원본을 마우스 오른쪽 단추로 클릭한 다음 **데이터 세트 추가**를 선택합니다.
    ...
    *보고서 데이터 세트는 Power BI 데이터 세트와 용도 및 구조가 다릅니다.*
...
1. **보고서 데이터** 창에서 매개 변수** 폴더를 **확장하여 SalesOrderNumber** 보고서 매개 변수를 표시**합니다.
...
6. **값** 드롭다운 목록에서 텍스트를 **43659**로 바꿉니다.
    ...
    *판매 주문 43659는 처음에 보고서 디자인을 테스트하는 데 사용할 값입니다.*

7. **확인**을 선택합니다.

8. 보고서를 저장합니다.

    *이제 판매 주문을 설명하는 텍스트 상자를 추가하여 보고서 헤더 영역 디자인을 완료합니다.*
...
35. 디자인 보기로 돌아가려면 **실행** 리본 탭의 **보기** 그룹 안에서 **디자인**을 선택합니다.
    ...
    *이제 보고서 본문에 표를 추가하여 판매 주문 라인의 서식이 지정된 레이아웃을 표시합니다.*
...
12. 보고서를 미리 봅니다.
    ...
    *다음 작업에서는 다음 작업을 수행합니다.*

    - *배경색과 굵은 글꼴 스타일을 사용하여 표 머리글 서식 지정*

    - *열 너비를 수정하여 중복 공간을 제거하고 긴 텍스트 값이 래핑되지 않도록 방지*

    - *첫 번째 열 값 왼쪽 맞춤*

    - *마지막 세 열 값 오른쪽 맞춤*

    - *통화 기호(USD)를 사용하여 통화 값 서식 지정*
...
23. 보고서를 다시 실행하려면 오른쪽에서 **보고서 보기**를 선택합니다.
    ...
    *이 판매 주문에는 72개의 판매 주문 라인이 있으므로 데이터가 여러 페이지에 렌더링됩니다.*
...
5. **속성** 창에서 **기타** > **RepeatOnNewPage** 속성을 **True**로 설정합니다.
    ...
    *이렇게 하면 테이블 머리글을 나타내는 첫 번째 정적 그룹이 모든 페이지에서 반복됩니다.*
...
14. 보고서 본문을 선택하려면 테이블 텍스트 상자를 마우스 오른쪽 단추로 클릭한 다음 **선택** > **본문**을 선택합니다.
    ...
    *표가 전체 보고서 본문을 채우면 이 기술을 사용하여 보고서 본문을 선택해야 합니다.*
